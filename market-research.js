...
function updateRegionSection(regionsData) {
    ...
    // Apenas atualiza as cores do mapa existente
    applyStateColors(regionsData);
}
...
function applyStateColors(regionsData) {
    const sampleRegions = [
    ...
    dataToRender.forEach(region => {
        const path = document.getElementById(region.state.toUpperCase());
        if (path) {
            path.style.fill = getColorByPercentage(region.percentage);
            // Remover classes de cor antigas e adicionar a nova
            path.classList.remove("pessimo", "ruim", "fraco", "mediano", "bom", "excelente");
...
function getColorClass(percentage) {
    ...
    if (percentage > 30) return "mediano";
    if (percentage > 10) return "fraco";
    if (percentage > 5) return "ruim";
    return "pessimo";
}
...
function renderTrendChart(trendsData) {
    ...
    data.forEach((point, index) => {
        const x = padding + (index / (data.length - 1)) * chartWidth;
        ctx.fillText(point.period, x, padding + chartHeight + 20pContainer = document.getElementById('brazilMapContainer');
...
function initMap() {
    ...
    // Carregar o SVG do mapa apenas uma vez na inicialização
